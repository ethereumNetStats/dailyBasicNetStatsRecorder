...
const recordTableName: string = "ethereum.dailyBasicNetStats";

// socket.io-clientの定義
const socketClientName: string = "dailyBasicNetStatsRecorder";
...
socketClient.on("newBlockDataRecorded", async (blockNumberWithTimestamp: blockNumberWithTimestamp) => {
    ...
    if (!isRecording) {
        ...
        let [endTimeOfDailyBasicNetStats] = await pool.query<RowDataPacket[any]>(`SELECT endTimeUnix
                                                                                     FROM ${recordTableName}
                                                                                     ORDER BY endTimeUnix DESC LIMIT 1`);

        if (endTimeOfDailyBasicNetStats.length === 0) {
            // データベースが空の場合には0のタイムスタンプを設定
            endTimeOfDailyBasicNetStats[0] = {endTimeUnix: 0};
        }

        if (blockNumberWithTimestamp.timestamp) {
            // タイムスタンプの値がTruthyなら集計期間の開始時間と終了時間を示すtimeRangeArray配列を生成
            let timeRangeArray: timeRangeArray = timeRangeArrayMaker(endTimeOfDailyBasicNetStats[0].endTimeUnix, blockNumberWithTimestamp.timestamp, DURATION);
