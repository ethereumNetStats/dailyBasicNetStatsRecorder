...
export const recordBasicNetStats = async (timeRangeArray: timeRangeArray, socketClient: Socket<ClientToServerEvents>, recordTableName: string, durationInSec: number) => {
    console.log(`${currentTimeReadable()} | Start: Recording daily basic net stats process.`);
    ...
    for (let timeRange of timeRangeArray) {
        ...
        await pool.query<OkPacket>(`INSERT INTO ${recordTableName}
                                    SET ?`, queryData);
        console.log(`${currentTimeReadable()} | Insert : A daily basic net stats. | Datetime : ${unixTimeReadable(queryData.startTimeUnix)} - ${unixTimeReadable(queryData.endTimeUnix)}`);

        // 集計データの記録をsocketServerに通知
        socketClient.emit('dailyBasicNetStatsRecorded', queryData);
        console.log(`${currentTimeReadable()} | Emit : 'dailyBasicNetStatsRecorded' | To : socketServer`);

    }

    // 処理時間の計測終了
    let processEnd: number = performance.now();
    console.log(`${currentTimeReadable()} | End : Recording daily basic net stats process. | Process time : ${((processEnd - processStart) / 1000).toString().slice(0, -12)} sec.`);
